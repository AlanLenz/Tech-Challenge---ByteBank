...
import { colors } from "@shared/hooks/useThemeColors";
...
const variantStyles: Record<string, React.CSSProperties> = {
  ...
  secondary: { backgroundColor: "transparent", border: `2px solid ${colors.primary}`, color: colors.primary },
  outline: { backgroundColor: "transparent", border: `1px solid ${colors.primary}`, color: colors.primary },
  destructive: { backgroundColor: colors.red, color: colors.white },
  ghost: { backgroundColor: "transparent", color: colors.primary },
  neutral: { backgroundColor: "transparent", border: `1px solid ${colors.black}`, color: colors.black },
};
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", size = "md", loading = false, disabled, children, style, ...props }, ref) => {
    const Comp = "button";
    return (
      <Comp
        ref={ref}
