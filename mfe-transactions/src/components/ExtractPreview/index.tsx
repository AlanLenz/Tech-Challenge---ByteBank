...
const ExtractPreview = ({ transfers, isLoading = false }: ExtractPreviewProps) => {
  ...
  return (
    <section className="w-full rounded-lg p-4 min-h-[450px] flex flex-col justify-between gap-4" style={{ backgroundColor: white }}>
      <div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 flex flex-col gap-4">
            {lastTransfers.map((item) => (
              <div key={item.id}>
                {/* O seu código de renderização do item continua exatamente igual! */}
      ...
      <div className="text-end w-full">
        <Link href="/extract" style={{ color: black }} className="cursor-pointer font-bold text-[14px] underline">
          Ver mais!
        </Link>
      </div>
    </section>
